...
export default class UserController {
   async createUser(req: Request, res: Response): Promise<any> {
    try {
      ...
      console.log("Se va a crear el usuario por que no existe la F-", body.ficha)
...
if(response.ok)
   ...
   async deleteUser(req: Request, res: Response): Promise<any> {
    try {
      ...
      infoUser.data.status = infoUser.data.status === false ? true : false;
      infoUser.data.auditUsers = infoLoger?.data?.ficha !== undefined
